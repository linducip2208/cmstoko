...
- B8 THEME ENGINE DONE: config/theme-presets.php (7 presets: editorial/minimal/luxury/fashion/tech/market/bold — palette+font pairing+radius scale per preset), App\Support\Theme (cached resolved var set: active preset + admin color overrides), storefront <head> emits :root overrides after stylesheet (last-declaration wins), ManageTheme Filament page (Tampilan→Tema: radio presets w/ descriptions + 3 ColorPicker overrides, appearance.update gated, audited). ThemeTest(5): tokens render, instant switch, custom override, audit, customer 403.
- B4-MENU BUILDER DONE: menus + menu_items tables (nested, target custom/category/brand/page), MenuItem::resolvedUrl() (safe: internal paths or same-host only, missing/inactive targets skipped), Menu::activeAt(location) cached-forever + flush on save/delete, MenuResource admin w/ item repeater, header/mega-dropdown/mobile drawer/footer all consume menus with category fallback. MenuTest(5): fallback, replacement, dead targets + external host blocked, nested children render, inactive ignored.
